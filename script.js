...
/**
 * Atualizar estado visual do botão PT/EN
 */
...
/**
 * Notificação visual de troca de idioma
 */
...
function typeWriterEffect() {
    const messages = [
        'UNIFED – PROBATUM v13.1.0-ELITE-GOLD · DORA COMPLIANT · COURT READY initializing...',
        ...
        'IFDE - PROBATUM · v13.1.0-ELITE-GOLD · DORA COMPLIANT · COURT READY · Data Proxy: Fleet Extract'
    ];
    
    let i = 0;
    const interval = setInterval(() => {
        if (i < messages.length) {
            console.log(`%c[UNIFED] ${messages[i]}`, 'color: #4a90e2; font-family: monospace; font-weight: bold;');
            i++;
        } else {
            clearInterval(interval);
            console.log('%c[UNIFED] System ready. Waiting for evidence...', 'color: #c5a059; font-family: monospace;');
        }
    }, 800);
}

// ============================================================================
// 8. DYNAMIC HASH-TO-QR MAPPING — v13.1.0-ELITE-GOLD · DORA COMPLIANT
// ============================================================================
...
// Biblioteca: qrcode.js (CDN injectado no HTML antes deste script)
// ============================================================================
function generateRealQRCode() {
    const qrContainer = document.getElementById('qrcode-demo');
    if (!qrContainer) { return; }

    // Ler Master Hash SHA-256 do DOM (único source-of-truth)
    const hashEl = document.getElementById('masterHash');
    const masterHashValue = hashEl
        ? (hashEl.dataset.hash || hashEl.textContent.trim())
        : 'HASH_INDISPONIVEL';
    ...
    if (typeof QRCode !== 'undefined') {
        try {
            ...
            // Metadados forenses no container
            qrContainer.setAttribute('data-hash',    masterHashValue);
            qrContainer.setAttribute('data-payload', qrPayload);
            qrContainer.setAttribute('aria-label',
                'QR Code SHA-256: ' + masterHashValue.substring(0, 16) + '...');
            qrContainer.setAttribute('title',
                'Master Hash SHA-256 (UNIFED – PROBATUM v13.1.0-ELITE-GOLD)\n' + masterHashValue);
            qrContainer.setAttribute('data-tooltip',
                'QR Code de integridade SHA-256 · Clique para copiar hash');
            qrContainer.style.cursor = 'pointer';

            // Click → copia hash para validação independente
            qrContainer.addEventListener('click', function onQRClick() {
                if (navigator.clipboard) {
                    navigator.clipboard.writeText(masterHashValue)
                        .then(function() {
                            showToast('Master Hash SHA-256 copiado · Valide em qualquer verificador', 'success');
                        })
                        .catch(function() {
                            showToast('Hash: ' + masterHashValue.substring(0, 20) + '...', 'info');
                        });
                }
            });

            console.log('%c[UNIFED] QR Code SHA-256 gerado · ' + masterHashValue.substring(0, 16) + '...',
                'color: #4a90e2; font-family: monospace;');

        } catch (qrErr) {
            // Fallback forense: hash em texto se qrcode.js falhar
            qrContainer.innerHTML =
            ...
            console.warn('[UNIFED] QRCode lib error — hash como texto:', qrErr);
        }
    } else {
        ...
        qrContainer.innerHTML =
            ...
            + 'word-break:break-all;color:#4a90e2;display:block;padding:4px;">'
            + masterHashValue + '</span>';
        console.warn('[UNIFED] QRCode library not loaded — fallback texto activo');
    }
}

// Alias de retrocompatibilidade (garante que chamadas legacy não quebram)
function simulateQRCode() { generateRealQRCode(); }
...
const protocolContent = {
    privacidade: {
        ...
        body: `<h3>Dever de Sigilo Profissional · Art. 135.º CP</h3>
               <p>A IFDE - PROBATUM opera sob o princípio da <strong>inviolabilidade do segredo pericial</strong>. Todos os dados recolhidos no âmbito do Sistema UNIFED – PROBATUM v13.1.0-ELITE-GOLD são processados em ambientes segregados (Air-Gapped quando necessário), garantindo a confidencialidade absoluta das informações dos mandatários.</p>
               <ul>
    ...
    termos: {
        ...
        body: `<h3>Enquadramento Jurídico da Peritagem</h3>
               <p>Os serviços prestados pela IFDE - PROBATUM constituem <strong>perícia técnica de assistência</strong> conforme os art. 467.º a 489.º do Código de Processo Civil e art. 151.º a 157.º do Código de Processo Penal. O Sistema UNIFED – PROBATUM v13.1.0-ELITE-GOLD é uma ferramenta de apoio à decisão e não substitui o juízo crítico do perito nomeado pelo tribunal.</p>
               <ul>
...
function openProtocol(id) {
    ...
    if (modal && title && text && protocolContent[id]) {
        ...
        console.log(`%c[UNIFED] Protocolo aberto: ${id} - ${protocolContent[id].title}`, 'color: #c5a059; font-family: monospace;');
    }
}
...
function openForensicReport() {
    ...
    const reportTemplate = `
        ...
        <div class="report-body">
            ...
            <p>Análise de fluxo de ativos e auditoria algorítmica ao Sistema de Faturação da entidade sob investigação, com recurso ao protocolo <strong>UNIFED – PROBATUM (Intelligent Forensic Digital Evidence)</strong> v13.1.0-ELITE-GOLD. O perímetro de análise compreende o período fiscal de 2025, com base nos ficheiros SAF-T (PT) e extratos de plataforma fornecidos.</p>
            ...
            <h2>3. ANÁLISE DE DISCREPÂNCIAS (UNIFED – PROBATUM)</h2>
            ...
            <p>Com base na evidência digital recolhida e processada pelo motor forense UNIFED – PROBATUM v13.1.0-ELITE-GOLD, conclui-se pela <strong>existência de quebra na integridade dos dados fiscais</strong>, configurando indícios de infração prevista no Regime Geral das Infrações Tributárias (RGIT), nomeadamente:</p>
    ...
    console.log('%c[UNIFED] Visualizador de relatório forense aberto', 'color: #4a90e2; font-family: monospace;');
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log('%cUNIFED – PROBATUM v13.1.0-ELITE-GOLD · DORA COMPLIANT loaded.', 'color: #4a90e2; font-family: monospace;');
